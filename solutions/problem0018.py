...
def main():
    """Main function."""
    grid = findall(r"(\d\d(?: \d\d)*)<br>", problem["description"])
    grid = [[int(number) for number in row.split(' ')] for row in grid]
    new_row = []
    for row, numbers in enumerate(grid):
        old_row, new_row = new_row, []
        for col, number in enumerate(numbers):
            left = old_row[col - 1] if 0 < col else 0
            right = old_row[col] if col < row else 0
            new_row.append(max(left, right) + number)
    answer = max(new_row)
    return answer
