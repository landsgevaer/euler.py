...
def main():
    """Main function."""
    grid = findall(r"(\d\d(?: \d\d){19})<br>", problem["description"])
    grid = [[int(number) for number in row.split(" ")] for row in grid]
    answer = 0
    for row in range(20):
        for col in range(20):
            if row < 17:
                answer = max(answer, prod(grid[row + i][col] for i in range(4)))
            if col < 17:
                answer = max(answer, prod(grid[row][col + i] for i in range(4)))
            if row < 17 and col < 17:
                answer = max(answer, prod(grid[row + i][col + i] for i in range(4)))
                answer = max(answer, prod(grid[row + i][col + 3-i] for i in range(4)))
    return answer
