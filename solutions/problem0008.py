...
def main():
    """Main function."""
    number = ''.join(findall(r"(\d{50})<br>", problem["description"]))
    answer = 0
    for start in range(len(number) - 12):
        answer = max(answer, prod(int(number[pos]) for pos in range(start, start + 13)))
    return answer
